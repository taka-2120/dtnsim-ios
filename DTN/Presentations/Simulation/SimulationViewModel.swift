...
class SimulationViewModel {
    ...
    private func calculateDifference(for message: Message) -> CGPoint {
        ...
        if message.destination.position.y < message.start.position.y && 0 < slope {
            slope *= -1
        }
    ...
    private func startMovingMessage(for message: Message) {
        ...
        self.messages[index].timer = Timer.scheduledTimer(withTimeInterval: 0.02, repeats: true) { _ in
        ...
        self.messages[index].timer?.fire()
    }
}
